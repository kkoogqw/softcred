...
use super::{
    ConstraintSystem, LinearCombination, R1CSProof,
...
pub struct Verifier<T: BorrowMut<Transcript>> {
    ...
    deferred_constraints: Vec<
        Box<dyn FnOnce(&mut RandomizingVerifier<T>) -> Result<(), R1CSError>>,
    >,
...
impl<T: BorrowMut<Transcript>> ConstraintSystem for Verifier<T> {
    fn transcript(&mut self) -> &mut Transcript { self.transcript.borrow_mut() }
    ...
    // New items
    fn evaluate_lc(&self, _: &LinearCombination) -> Option<Scalar> { None }
    ...
    fn allocate_single(
        ...
        match var {
            Variable::MultiplierLeft(i) => {
                Ok((Variable::MultiplierLeft(i), None))
            },
...
impl<T: BorrowMut<Transcript>> ConstraintSystem for RandomizingVerifier<T> {
    ...
    fn metrics(&self) -> Metrics { self.verifier.metrics() }
...
impl<T: BorrowMut<Transcript>> RandomizedConstraintSystem
    for RandomizingVerifier<T>
{
...
impl<T: BorrowMut<Transcript>> Verifier<T> {
    ...
    fn create_randomized_constraints(mut self) -> Result<Self, R1CSError> {
        ...
        if self.deferred_constraints.len() == 0 {
            ...
            // but specifying lifetimes for boxed closures is not going to be
            // nice, so we move the self into wrapper and then move
            // it back out afterwards.
            let mut callbacks =
                mem::replace(&mut self.deferred_constraints, Vec::new());
    ...
    pub fn verify_and_return_transcript(
        ...
        // We cannot do this in advance because user can commit variables
        // one-by-one, but this suffix provides safe disambiguation
        // because each variable is prefixed with a separate label.
        ...
        // If the number of multiplications is not 0 or a power of 2, then pad
        // the circuit.
        let n = self.num_vars;
        let n2 = n - n1;
