use crate::utils::r1cs::AllocatedScalar;
...
pub fn is_zero_gadget<CS: ConstraintSystem>(
	...
	let x_lc: LinearCombination =
		vec![(x.variable, Scalar::ONE)].iter().collect();
	let one_minus_y_lc: LinearCombination =
		vec![(Variable::One(), Scalar::from(1 - y))]
			.iter()
			.collect();
	let y_lc: LinearCombination =
		vec![(Variable::One(), Scalar::from(y))].iter().collect();
	let inv_lc: LinearCombination =
		vec![(Variable::One(), Scalar::from(inv))].iter().collect();
...
pub fn is_nonzero_gadget<CS: ConstraintSystem>(
	...
	let one_minus_y_lc =
		LinearCombination::from(Variable::One()) - y_lc.clone();
	...
	// x * x_inv = y
	let inv_lc: LinearCombination =
		vec![(x_inv.variable, Scalar::ONE)].iter().collect();
