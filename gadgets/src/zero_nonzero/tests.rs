...
// 			let (com_val, var_val) =
// 				prover.commit(value.clone(), Scalar::random(&mut rng));
...
// 			let (com_val_inv, var_val_inv) =
// 				prover.commit(inv.clone(), Scalar::random(&mut rng));
// 			let alloc_scal_inv = AllocatedScalar {
// 				variable: var_val_inv,
// 				assignment: Some(inv),
// 			};
// 			assert!(is_nonzero_gadget(&mut prover, alloc_scal, alloc_scal_inv)
// 				.is_ok());
...
// 		assert!(is_nonzero_gadget(&mut verifier, alloc_scal, alloc_scal_inv)
// 			.is_ok());
