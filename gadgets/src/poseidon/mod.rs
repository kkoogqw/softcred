...
use crate::{
    ...
    utils::{
        r1cs::{constrain_lc_with_scalar, AllocatedScalar},
        scalar::get_scalar_from_hex,
    },
    zero_nonzero::{is_nonzero_gadget, is_zero_gadget},
};
use core::borrow::BorrowMut;
use merlin::Transcript;
use std::{collections::HashMap, mem};

use crate::poseidon::constants::x3_2::{MDS_ENTRIES, ROUND_CONSTS};
...
pub fn Poseidon_permutation(
        ...
        for j in 0..width {
            for i in 0..width {
                current_state_temp[i] +=
                    current_state[j] * params.mds_matrix[i][j];
            }
        }
    ...
    for _ in full_rounds_beginning..(full_rounds_beginning + partial_rounds) {
        ...
        current_state[width - 1] =
            params.sbox.apply_sbox(&current_state[width - 1]);
        ...
        for j in 0..width {
            for i in 0..width {
                next_inputs[i] = next_inputs[i].clone()
                    + sbox_outs[j].clone() * mds_matrix[i][j];
            }
        }
    }
    ...
    for _k in full_rounds_beginning..(full_rounds_beginning + partial_rounds) {
        ...
        for i in 0..width {
            ...
            if i == width - 1 {
                ...
                sbox_outputs[i] =
                    input_vars[i].clone() + LinearCombination::from(round_key);
            }
...
pub fn Poseidon_permutation_gadget<'a, CS: ConstraintSystem>(
    ...
    let input_vars: Vec<LinearCombination> =
        input.iter().map(|e| e.variable.into()).collect();
    let permutation_output =
        Poseidon_permutation_constraints::<CS>(cs, input_vars, params)?;

    for i in 0..width {
        constrain_lc_with_scalar::<CS>(
            cs,
            permutation_output[i].to_owned(),
            &output[i],
        );
    }
...
pub fn Poseidon_hash_2_constraints<'a, CS: ConstraintSystem>(
    ...
    let permutation_output =
        Poseidon_permutation_constraints::<CS>(cs, inputs, params)?;
...
pub fn Poseidon_hash_4_gadget<'a, CS: ConstraintSystem>(
    ...
    let hash =
        Poseidon_hash_4_constraints::<CS>(cs, input_arr, statics, params)?;
...
pub fn allocate_statics_for_prover<T: BorrowMut<Transcript>>(
    prover: &mut Prover<T>,
    num_statics: usize,
) -> Vec<AllocatedScalar> {
    let mut statics = vec![];
...
pub fn allocate_statics_for_verifier<T: BorrowMut<Transcript>>(
